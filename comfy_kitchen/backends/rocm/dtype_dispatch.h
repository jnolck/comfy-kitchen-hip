...
template<> struct DTypeMap<DTYPE_CODE_BFLOAT16> { using type = __hip_bfloat16; };

// Type mapping for FP8 output dtype codes
template<int DTypeCode> struct FP8DTypeMap;

template<> struct FP8DTypeMap<DTYPE_CODE_FLOAT8_E4M3FN> { using type = __hip_fp8_e4m3_fnuz; };
template<> struct FP8DTypeMap<DTYPE_CODE_FLOAT8_E5M2> { using type = __hip_fp8_e5m2_fnuz; };
...
inline hipDataType dtype_code_to_cuda_type(int dtype_code) {
    switch (dtype_code) {
        case DTYPE_CODE_FLOAT32: return HIP_R_32F;
        case DTYPE_CODE_FLOAT16: return HIP_R_16F;
        case DTYPE_CODE_BFLOAT16: return HIP_R_16BF;
        default:
