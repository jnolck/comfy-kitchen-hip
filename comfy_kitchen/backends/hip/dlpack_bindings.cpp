...
//     const void *x, const void *smooth, const void *lora_down, void *q_x,
//     void *ascales, void *lora_act, int M, int M_pad, int K, int R,
//     int input_dtype_code, int act_unsigned, hipStream_t stream);
//
...
void stochastic_round_fp8(nb::ndarray<nb::device::rocm> rng_and_output,
                          nb::ndarray<nb::device::rocm> input,
...
//     nb::ndarray<uint8_t, nb::ndim<2>, nb::device::rocm> b,
...
//     nb::ndarray<nb::device::rocm> bias, nb::ndarray<nb::device::rocm>
//     workspace, bool accumulate, nb::ndarray<float, nb::device::rocm> alpha,
//     uintptr_t stream_ptr) {
//
...
//                     nb::ndarray<nb::device::rocm> output,
//                     nb::ndarray<nb::device::rocm> block_scales, float
...
//                     nb::ndarray<nb::device::rocm> output,
//                     nb::ndarray<nb::device::rocm> block_scales, bool pad_32x,
//                     uintptr_t stream_ptr) {
//
//   // Get input dimensions (orig_rows, orig_cols)
//   int64_t orig_rows = input.shape(0);
...
void apply_rope(nb::ndarray<nb::device::rocm> xq,
                nb::ndarray<nb::device::rocm> freqs,
                nb::ndarray<nb::device::rocm> xq_out, nb::object xk_obj,
...
void adaln(nb::ndarray<nb::device::rocm> x, nb::ndarray<nb::device::rocm> scale,
           nb::ndarray<nb::device::rocm> shift,
           nb::ndarray<nb::device::rocm> out, int64_t N, int64_t D,
           int64_t scale_group, int64_t shift_group, float eps, int dtype_code,
           uintptr_t stream_ptr) {
  hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
...
void cublas_gemm_int8(nb::ndarray<int8_t, nb::ndim<2>, nb::device::rocm> a,
                      ...
                      nb::ndarray<nb::device::rocm> workspace,
                      uintptr_t stream_ptr) {
...
void quantize_int8_rowwise(
    ...
    nb::ndarray<float, nb::ndim<2>, nb::device::rocm> scales, bool stochastic,
...
void rotate_int8_convrot_weight(
    nb::ndarray<nb::ndim<2>, nb::device::rocm> input,
    nb::ndarray<nb::ndim<2>, nb::device::rocm> output, int64_t group_size,
    uintptr_t stream_ptr) {
  ...
  launch_rotate_int8_convrot_weight_kernel(
      input.data(), output.data(), M, K, static_cast<int>(group_size),
      input_dtype_code, output_dtype_code, stream);
}
...
void quantize_int8_convrot_staged(
  ...
  if (input_dtype_code < 0 || input_dtype_code > 2 || rotated_dtype_code < 0 ||
    ...
    throw std::runtime_error(
        "Unsupported dtype for ConvRot staged quantization");
  }

  hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
  launch_quantize_int8_convrot_staged_kernel(
      input.data(), rotated.data(), partial_absmax.data(), output.data(),
      scales.data(), M, K, static_cast<int>(group_size), input_dtype_code,
...
void dequantize_int8_linear(
    nb::ndarray<int32_t, nb::ndim<2>, nb::device::rocm> input,
...
void int8_gemv_dequant(
  ...
  launch_int8_gemv_dequant_kernel(
      ...
      has_bias ? bias.data() : nullptr, output.data(), N, K,
      static_cast<int64_t>(weight_scales.size()), has_bias, output_dtype_code,
      bias_dtype_code, stream);
}
...
void int8_linear_m1(
  ...
  if (convrot && (group_size != 256 || K % 256 != 0)) {
    throw std::runtime_error("INT8 M=1 ConvRot linear requires group_size 256 "
                             "and K divisible by 256");
  }
  ...
  if (convrot) {
    launch_quantize_int8_rowwise_convrot64_kernel(
        input.data(), q_scratch.data(), x_scales.data(), M, K, group_size,
        input_dtype_code, false, 0, stream);
  } else {
    launch_quantize_int8_rowwise_kernel(input.data(), q_scratch.data(),
                                        x_scales.data(), M, K, input_dtype_code,
                                        false, 0, stream);
  }
  launch_int8_gemv_dequant_kernel(
      q_scratch.data(), weight.data(), x_scales.data(), weight_scales.data(),
      has_bias ? bias.data() : nullptr, output.data(), N, K,
      static_cast<int64_t>(weight_scales.size()), has_bias, output_dtype_code,
      bias_dtype_code, stream);
}
...
void dequantize_int8_simple(nb::ndarray<int8_t, nb::device::rocm> input,
  ...
  if (output.size() != input.size()) {
    throw std::runtime_error(
        "INT8 simple dequantization output shape mismatch");
  }
  if (scale_mode == 0 && scale.size() != 1) {
    throw std::runtime_error(
        "INT8 simple dequantization scalar scale shape mismatch");
  }
  ...
  if (scale_mode == 2 && (inner_dim <= 0 || input.size() % inner_dim != 0 ||
    ...
    throw std::runtime_error(
        "INT8 simple dequantization rowwise scale shape mismatch");
  }
  const int output_dtype_code = map_dtype_to_code(output.dtype());
  if (output_dtype_code < 0 || output_dtype_code > 2) {
    throw std::runtime_error(
        "Unsupported output dtype for INT8 simple dequantization");
  }

  hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
  launch_dequantize_int8_simple_kernel(
      input.data(), scale.data(), output.data(),
      static_cast<int64_t>(input.size()), inner_dim, scale_mode,
      output_dtype_code, stream);
}
...
void dequantize_int8_convrot_weight(
  ...
  if (scale.size() != 1 && scale.size() != static_cast<size_t>(M)) {
    throw std::runtime_error(
        "INT8 convrot dequant scale must be scalar or per-row");
  }
  const int output_dtype_code = map_dtype_to_code(output.dtype());
  if (output_dtype_code < 0 || output_dtype_code > 2) {
    throw std::runtime_error(
  ...
  launch_dequantize_int8_convrot_kernel(
      input.data(), scale.data(), output.data(), M, K,
      static_cast<int64_t>(scale.size()), static_cast<int>(group_size),
      output_dtype_code, stream);
}

NB_MODULE(_C, m) {
  m.doc() = "comfy_kitchen CUDA kernels - nanobind + DLPack interface (NO "
  ...
  m.def("quantize_int8_rowwise", &quantize_int8_rowwise,
        ...
        nb::arg("output"), nb::arg("scales"), nb::arg("stochastic"),
  ...
  m.def("quantize_int8_rowwise_convrot", &quantize_int8_rowwise_convrot,
        ...
        nb::arg("group_size"), nb::arg("stochastic"), nb::arg("seed"),
        nb::arg("stream_ptr"));
  ...
  m.def("quantize_int8_convrot_staged", &quantize_int8_convrot_staged,
        "ConvRot rotation with partial absmax followed by INT8 rowwise "
        "quantization",
        nb::arg("input"), nb::arg("rotated"), nb::arg("partial_absmax"),
        nb::arg("output"), nb::arg("scales"), nb::arg("group_size"),
  ...
  m.def("int8_gemv_dequant", &int8_gemv_dequant,
        "INT8 GEMV with fused rowwise x colwise dequantization, bias, and "
        "output cast",
        nb::arg("input"), nb::arg("weight"), nb::arg("x_scales"),
  ...
  m.def("int8_linear_m1", &int8_linear_m1,
        ...
        nb::arg("input"), nb::arg("q_scratch"), nb::arg("x_scales"),
        nb::arg("weight"), nb::arg("weight_scales"), nb::arg("bias"),
        nb::arg("output"), nb::arg("output_dtype_code"), nb::arg("convrot"),
        nb::arg("group_size"), nb::arg("stream_ptr"));

  m.def("dequantize_int8_simple", &dequantize_int8_simple,
        "INT8 dequantization to float32", nb::arg("input"), nb::arg("scale"),
        nb::arg("output"), nb::arg("inner_dim"), nb::arg("scale_mode"),
        nb::arg("stream_ptr"));

  m.def("dequantize_int8_convrot_weight", &dequantize_int8_convrot_weight,
        "INT8 ConvRot weight dequantization to float32", nb::arg("input"),
        nb::arg("scale"), nb::arg("output"), nb::arg("group_size"),
        nb::arg("stream_ptr"));
