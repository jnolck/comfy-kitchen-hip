...
//     const void* bias_ptr = (bias.data() && bias.size() > 0) ? bias.data() :
//     nullptr;
//
//     // Call the kernel
...
//     int input_dtype_code = map_dtype_to_code(input.dtype());
//     if (input_dtype_code < 0 || input_dtype_code > 2) {
//         throw std::runtime_error("Unsupported input dtype for FP4
//         quantization (must be float32, float16, or bfloat16)");
//     }
//
//     hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
...
//     // Validate output dtype code (0=float32, 1=float16, 2=bfloat16)
//     if (output_dtype_code < 0 || output_dtype_code > 2) {
//         throw std::runtime_error("Unsupported output dtype for FP4
//         dequantization (must be float32, float16, or bfloat16)");
//     }
//
//     hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
...
//     int input_dtype_code = map_dtype_to_code(input.dtype());
//     if (input_dtype_code < 0 || input_dtype_code > 2) {
//         throw std::runtime_error("Unsupported input dtype for MXFP8
//         quantization (must be float32, float16, or bfloat16)");
//     }
//
//     hipStream_t stream = reinterpret_cast<hipStream_t>(stream_ptr);
...
//     static_cast<int>(wgt.shape(0)); int R =
//     static_cast<int>(lora_act_in.shape(1)); int out_code =
//     svdquant_dtype_code(out.dtype()); if (fuse_lora &&
//     svdquant_dtype_code(lora_act_in.dtype()) != out_code) {
//         throw std::runtime_error(
//             "svdquant_scaled_mm_w4a4: fused LoRA-up requires lora_act_in
//             dtype " "to match output/lora_up dtype");
//     }
//
//     if (tile_packed) {
//         if (wgt.shape(1) != K / 64 || wgt.shape(2) != 32 || wgt.shape(3) !=
//         128) {
//             throw std::runtime_error(
...
//     }
//
//     const void* bias_ptr = (bias.data() != nullptr && bias.size() > 0) ?
//     bias.data() : nullptr; hipStream_t stream =
...
//         throw std::runtime_error("awq_w4a16: only fp16 (1) and bf16 (2)
...
NB_MODULE(_C, m) {
  ...
  //       false -> fall back to cuBLAS", nb::arg("a"), nb::arg("b"),
  //       nb::arg("xs"),
  //       nb::arg("ws"),
  //       nb::arg("bias"),
  //       nb::arg("d"),
