...
template <typename T> struct FP8LimitsTrait;

template <> struct FP8LimitsTrait<__hip_fp8_e4m3_fnuz> {
  static constexpr float max = 448.0f;
  static constexpr float max_inverse = 1.0 / max;
};

template <> struct FP8LimitsTrait<__hip_fp8_e5m2_fnuz> {
...
#if CUDA_VERSION >= 12080
//  FP4 type traits
template <typename T> struct FP4LimitsTrait;

template <> struct FP4LimitsTrait<__hip_fp4x2_storage_t> {
...
__forceinline__ __device__ void store_fp4x2(OType *output, size_t idx,
...
template <typename OType, bool hi_first = true>
__forceinline__ __device__ void store_fp4x4(OType *output, size_t idx,
  ...
  packed.fp4x2[0] =
      __hip_cvt_float2_to_fp4x2(args0, __HIP_E2M1, hipRoundNearest);
  packed.fp4x2[1] =
      __hip_cvt_float2_to_fp4x2(args1, __HIP_E2M1, hipRoundNearest);
