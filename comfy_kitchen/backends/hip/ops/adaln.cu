...
__device__ __forceinline__ float2 warp_reduce_sum2(float2 v)
{
        ...
        {
                v.x += __shfl_down_sync(0x00000000ffffffffull, v.x, offset);
                v.y += __shfl_down_sync(0x00000000ffffffffull, v.y, offset);
        }
        return v;
}
...
template <typename T>
__global__ void adaln_kernel(const T* __restrict__ x, const T* __restrict__ scale,
                             ...
                             int scale_group, int shift_group, float eps)
{
        ...
        const int shift_row =
            (shift_group == scale_group) ? scale_row : modulation_row(row, shift_group, n_rows);
...
#pragma unroll
        ...
        for (int i = vec_end + tid; i < D; i += nthreads)
        {
                float result = (to_float(x_row[i]) - mean) * rstd * (1.0f + to_float(s_row[i])) +
                               to_float(sh_row[i]);
...
template <typename T>
__global__ void adaln_warp_kernel(const T* __restrict__ x, const T* __restrict__ scale,
                                  const T* __restrict__ shift, T* __restrict__ out, int N, int D,
                                  int scale_group, int shift_group, float eps)
{
        ...
        const int shift_row =
            (shift_group == scale_group) ? scale_row : modulation_row(row, shift_group, N);
        ...
        const float sum = __shfl_sync(0x00000000ffffffffull, acc.x, 0);
        const float sumsq = __shfl_sync(0x00000000ffffffffull, acc.y, 0);
        const float inv_d = 1.0f / static_cast<float>(D);
...
extern "C"
{
        ...
        {
                if (N > std::numeric_limits<int>::max() || D > std::numeric_limits<int>::max() ||
                ...
                DISPATCH_FP_DTYPE(
                    ...
                    [&]()
                    {
                            if (N <= 1024)
                            {
                                    dim3 warp_grid(static_cast<unsigned int>(
                                        (N + comfy::kAdaLNRowsPerWarpBlock - 1) /
                            ...
                            if (err != hipSuccess)
                                    throw std::runtime_error(
                                        std::string("adaln kernel launch failed: ") +
