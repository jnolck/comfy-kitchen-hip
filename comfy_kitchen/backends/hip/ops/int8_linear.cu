...
template <typename T>
__device__ __forceinline__ float quant_div_float_to_float(float val,
                                                          float scale) {
  const float scale_t = to_float(from_float<T>(scale));
...
template <>
__device__ __forceinline__ float stochastic_sum_to_float<float>(float scaled,
                                                                float rng) {
  return scaled + rng;
}

template <>
__device__ __forceinline__ float stochastic_sum_to_float<half>(float scaled,
                                                               half rng) {
...
stochastic_sum_to_float<__hip_bfloat16>(float scaled, __hip_bfloat16 rng) {
...
__device__ __forceinline__ T stochastic_rng_value(int64_t idx, uint64_t seed) {
  ...
  const uint32_t folded =
      static_cast<uint32_t>(key) ^ static_cast<uint32_t>(key >> 32);
  const float value = static_cast<float>(pcg_hash(folded) >> 8) * 0x1.0p-24f;
  return from_float<T>(value);
}

template <typename T>
__device__ __forceinline__ void store4_contiguous(T *out, int64_t idx, float x,
...
store4_contiguous<float>(float *out, int64_t idx, float x, float y, float z,
                         float w) {
  reinterpret_cast<float4 *>(out)[idx / 4] = make_float4(x, y, z, w);
}

template <>
__device__ __forceinline__ void store4_contiguous<half>(half *out, int64_t idx,
                                                        float x, float y,
                                                        float z, float w) {
...
template <>
__device__ __forceinline__ void
store4_contiguous<__hip_bfloat16>(__hip_bfloat16 *out, int64_t idx, float x,
                                  float y, float z, float w) {
  reinterpret_cast<__hip_bfloat162 *>(out)[idx / 2] =
      //    __floats2bfloat162_rn(x, y);
      __lows2bfloat162(x, y);
  reinterpret_cast<__hip_bfloat162 *>(out)[idx / 2 + 1] =
      //    __floats2bfloat162_rn(z, w);
      __lows2bfloat162(z, w);
}
...
template <int NUM_WARPS>
__device__ __forceinline__ int block_reduce_sum_i32_t(int v, int *warp_smem,
                                                      int *block_smem) {
...
__global__ void quantize_int8_rowwise_convrot_kernel(
    const InputType *__restrict__ x, int8_t *__restrict__ q,
...
#pragma unroll
  ...
  for (int col = tid; col < K; col += BLOCK_THREADS) {
    ...
    const float scaled =
        quant_div_float_to_float<InputType>(row_buf[col], scale);
    float quantized;
    if constexpr (STOCHASTIC) {
      const InputType noise = stochastic_rng_value<InputType>(idx, seed);
      quantized = floorf(stochastic_sum_to_float<InputType>(scaled, noise));
...
__global__ void dequantize_int8_linear_vec4_kernel(
    const int32_t *__restrict__ input, const float *__restrict__ x_scales,
    const float *__restrict__ weight_scales, OutputType *__restrict__ output,
...
__global__ void int8_gemv_dequant_kernel(
    ...
    const BiasType *__restrict__ bias, OutputType *__restrict__ output, int N,
    int K, int weight_scale_size, bool has_bias) {
  constexpr int kWarps = BLOCK_THREADS / kThreadsPerWarp;
  ...
  for (int k4 = tid; k4 < K4; k4 += BLOCK_THREADS) {
    ...
    acc = __builtin_amdgcn_sudot4(true, x4[k4], true, w4[k4], acc, false);
  }
...
__global__ void dequantize_int8_simple_kernel(const int8_t *__restrict__ input,
  ...
  output[idx] = from_float<OutputType>(static_cast<float>(input[idx]) *
                                       scales[scale_idx]);
}

template <typename OutputType>
__global__ void dequantize_int8_simple_vec4_kernel(
    const int8_t *__restrict__ input, const float *__restrict__ scales,
    OutputType *__restrict__ output, int64_t total_vec4, int inner_dim_vec4,
    int scale_mode) {
  const int64_t idx4 =
      static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (idx4 >= total_vec4) {
...
__global__ void dequantize_int8_rowwise_vec4_2d_kernel(
    const int8_t *__restrict__ input, const float *__restrict__ scales,
    OutputType *__restrict__ output, int rows, int inner_dim_vec4) {
...
__global__ void dequantize_int8_convrot_groups_kernel(
    ...
    OutputType *__restrict__ output, int K, int scale_size) {
  constexpr int kGroupsPerBlock = BLOCK_THREADS / kConvRotGroup;
  extern __shared__ float smem[];

  const int group = static_cast<int>(blockIdx.x) * kGroupsPerBlock +
                    threadIdx.x / kConvRotGroup;
  const int row = static_cast<int>(blockIdx.y);
...
template <int S>
__device__ __forceinline__ void
convrot_fht_stage64(const float *__restrict__ src, float *__restrict__ dst,
                    int lane) {
...
__device__ __forceinline__ void
convrot_fht_stage64_store(const float *__restrict__ src,
...
template <int S, typename OutputType>
__device__ __forceinline__ float
convrot_fht_stage64_store_absmax(const float *__restrict__ src,
                                 OutputType *__restrict__ output, int lane) {
  const int base = (lane % S) + (lane / S) * (4 * S);
  const float x0 = src[base];
  const float x1 = src[base + S];
...
__global__ void dequantize_int8_convrot_groups64_kernel(
  ...
  if (active) {
    convrot_fht_stage64_store<64, OutputType>(
...
__global__ void rotate_int8_convrot_groups64_amax_kernel(
    const InputType *__restrict__ x, OutputType *__restrict__ output,
...
template <int GROUPS_PER_BLOCK, typename InputType, typename OutputType>
__global__ void
rotate_int8_convrot_groups64_kernel(const InputType *__restrict__ x,
...
__global__ void quantize_int8_rowwise_from_partials_kernel(
    const InputType *__restrict__ x, const float *__restrict__ partial_absmax,
    int8_t *__restrict__ q, float *__restrict__ scales, int K, uint64_t seed) {
  constexpr int kWarps = BLOCK_THREADS / kThreadsPerWarp;
  __shared__ float warp_smem[kWarps];
  __shared__ float block_smem;
  ...
  const int n_groups = K / kConvRotGroup;
  const int64_t row_offset = static_cast<int64_t>(row) * K;
  const float *row_partials =
      partial_absmax + static_cast<int64_t>(row) * n_groups;
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
    auto launch = [&](auto kernel, int block_threads) {
      kernel<<<static_cast<unsigned int>(num_rows), block_threads, 0, stream>>>(
          ...
          static_cast<float *>(scales), static_cast<int>(num_cols), seed);
    };
    ...
    if (num_cols >= 4096 && num_rows != 1) {
      ...
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_kernel<InputType,
                                                   comfy::kInt8Threads, true>,
               comfy::kInt8Threads);
      } else {
        launch(comfy::quantize_int8_rowwise_kernel<InputType,
                                                   comfy::kInt8Threads, false>,
               comfy::kInt8Threads);
      }
    }
  });
...
void launch_quantize_int8_rowwise_convrot_kernel(
    ...
    int64_t num_cols, int group_size, int input_dtype_code, bool stochastic,
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
    ...
    if (wide) {
      if (stochastic) {
        launch(
            comfy::quantize_int8_rowwise_convrot_kernel<InputType, 1024, true>);
      } else {
      ...
      if (stochastic) {
        launch(
            comfy::quantize_int8_rowwise_convrot_kernel<InputType, 256, true>);
      } else {
        launch(
            comfy::quantize_int8_rowwise_convrot_kernel<InputType, 256, false>);
      }
    }
  });
...
void launch_rotate_int8_convrot_weight_kernel(const void *input, void *output,
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
    DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
      comfy::rotate_int8_convrot_groups64_kernel<groups_per_block, InputType,
                                                 OutputType>
          <<<grid, block_threads, smem_bytes, stream>>>(
              static_cast<const InputType *>(input),
              static_cast<OutputType *>(output), static_cast<int>(num_cols));
    });
  });
...
void launch_quantize_int8_convrot_staged_kernel(
    const void *input, void *rotated, void *partial_absmax, void *output,
    void *scales, int64_t num_rows, int64_t num_cols, int group_size,
    int input_dtype_code, int rotated_dtype_code, bool stochastic,
    uint64_t seed, hipStream_t stream) {
  if (num_rows == 0 || num_cols == 0) {
    return;
  }
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
    DISPATCH_FP_DTYPE(rotated_dtype_code, RotatedType, [&] {
      comfy::rotate_int8_convrot_groups64_amax_kernel<groups_per_block,
                                                      InputType, RotatedType>
  ...
  DISPATCH_FP_DTYPE(rotated_dtype_code, RotatedType, [&] {
    ...
    if (quant_threads == 512) {
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_from_partials_kernel<RotatedType,
                                                                 512, true>,
               512);
      } else {
        launch(comfy::quantize_int8_rowwise_from_partials_kernel<RotatedType,
      ...
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_from_partials_kernel<
...
void launch_quantize_int8_rowwise_convrot64_kernel(
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, InputType, [&] {
    ...
    if (num_rows == 1) {
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_single, true>,
               block_threads_single);
      } else {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_single, false>,
               block_threads_single);
      }
      ...
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_256, true>,
               block_threads_256);
      } else {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_256, false>,
               block_threads_256);
      }
      ...
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_2560, true>,
               block_threads_2560);
      } else {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_2560, false>,
               block_threads_2560);
      }
      ...
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_6144, true>,
               block_threads_6144);
      } else {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_6144, false>,
               block_threads_6144);
      }
    } else {
      if (stochastic) {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_multi, true>,
               block_threads_multi);
      } else {
        launch(comfy::quantize_int8_rowwise_convrot64_kernel<
                   InputType, block_threads_multi, false>,
               block_threads_multi);
      }
    }
  });
  ...
  DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
    if (!has_bias) {
      if ((num_cols & 3) == 0) {
        ...
        const int vec4_blocks =
            static_cast<int>((total_vec4 + kVec4Threads - 1) / kVec4Threads);
...
void launch_int8_gemv_dequant_kernel(
    const void *input, const void *weight, const void *x_scales,
    const void *weight_scales, const void *bias, void *output, int64_t num_cols,
    int64_t K, int64_t weight_scale_size, bool has_bias, int output_dtype_code,
    int bias_dtype_code, hipStream_t stream) {
  if (num_cols == 0 || K == 0) {
    return;
  }
  ...
  DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
      ...
      if ((K & 3) == 0) {
        ...
        comfy::int8_gemv_dequant_warp_kernel<kWarpsPerBlock, OutputType, float>
            <<<blocks, kWarpsPerBlock * comfy::kThreadsPerWarp, 0, stream>>>(
                ...
                static_cast<const float *>(weight_scales), nullptr,
                static_cast<OutputType *>(output), static_cast<int>(num_cols),
                static_cast<int>(K), static_cast<int>(weight_scale_size),
                false);
      } else {
        comfy::int8_gemv_dequant_kernel<comfy::kInt8Threads, OutputType, float>
            <<<static_cast<unsigned int>(num_cols), comfy::kInt8Threads, 0,
               stream>>>(static_cast<const int8_t *>(input),
                         static_cast<const int8_t *>(weight),
                         static_cast<const float *>(x_scales),
                         static_cast<const float *>(weight_scales), nullptr,
    ...
    DISPATCH_FP_DTYPE(bias_dtype_code, BiasType, [&] {
      if ((K & 3) == 0) {
        ...
        const unsigned int blocks = static_cast<unsigned int>(
            (num_cols + kWarpsPerBlock - 1) / kWarpsPerBlock);
        comfy::int8_gemv_dequant_warp_kernel<kWarpsPerBlock, OutputType,
                                             BiasType>
            <<<blocks, kWarpsPerBlock * comfy::kThreadsPerWarp, 0, stream>>>(
                static_cast<const int8_t *>(input),
                static_cast<const int8_t *>(weight),
                ...
                static_cast<const BiasType *>(bias),
                static_cast<OutputType *>(output), static_cast<int>(num_cols),
                static_cast<int>(K), static_cast<int>(weight_scale_size), true);
      } else {
        comfy::int8_gemv_dequant_kernel<comfy::kInt8Threads, OutputType,
                                        BiasType>
            <<<static_cast<unsigned int>(num_cols), comfy::kInt8Threads, 0,
               stream>>>(static_cast<const int8_t *>(input),
                         static_cast<const int8_t *>(weight),
      ...
      DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        comfy::dequantize_int8_rowwise_vec4_2d_kernel<OutputType>
            <<<grid, block_threads, 0, stream>>>(
                ...
                static_cast<OutputType *>(output), static_cast<int>(rows),
                inner_dim_vec4);
      });
  ...
  if (num_cols >= comfy::kConvRotGroup) {
    auto launch_groups = [&](auto groups_tag) {
      ...
      dim3 grid(static_cast<unsigned int>(group_blocks),
                static_cast<unsigned int>(num_rows));
      DISPATCH_FP_DTYPE(output_dtype_code, OutputType, [&] {
        comfy::dequantize_int8_convrot_groups64_kernel<groups_per_block,
                                                       OutputType>
            <<<grid, block_threads, smem_bytes, stream>>>(
                static_cast<const int8_t *>(input),
                static_cast<const float *>(scales),
