...
struct FusedDequantEpilogue {
  ...
  CK_TILE_HOST_DEVICE void operator()(E &e, const C &c, const D0 &d0,
                                      const D1 &d1, const D2 &d2) const {
    float val = ck_tile::type_convert<float>(c) *
...
struct FusedInt8GemmCKTile {
  ...
  using GemmShape = ck_tile::TileGemmShape<
      ck_tile::sequence<GemmConfig::M_Tile, GemmConfig::N_Tile,
                        GemmConfig::K_Tile>,
      ck_tile::sequence<GemmConfig::M_Warp, GemmConfig::N_Warp,
                        GemmConfig::K_Warp>,
      ck_tile::sequence<GemmConfig::M_Warp_Tile, GemmConfig::N_Warp_Tile,
                        GemmConfig::K_Warp_Tile>>;

  using TilePartitioner = ck_tile::GemmSpatiallyLocalTilePartitioner<
      GemmShape, GemmConfig::TileParitionerGroupNum,
      GemmConfig::TileParitionerM01>;

  using GemmUniversalTraits = ck_tile::TileGemmUniversalTraits<
      GemmConfig::kPadM, GemmConfig::kPadN, GemmConfig::kPadK,
      GemmConfig::DoubleSmemBuffer, ALayout, BLayout, ELayout,
  ...
  using GemmEpilogue =
      ck_tile::CShuffleEpilogue<ck_tile::CShuffleEpilogueProblem<
          ADataType, BDataType, DsDataType, AccDataType, CDataType, DsLayout,
          ELayout, FusedDequantEpilogue, TilePartitioner::MPerBlock,
          TilePartitioner::NPerBlock, GemmConfig::M_Warp, GemmConfig::N_Warp,
          GemmConfig::M_Warp_Tile, GemmConfig::N_Warp_Tile,
          GemmConfig::K_Warp_Tile, UniversalGemmProblem::TransposeC>>;

  using Kernel =
      ck_tile::GemmKernelMultiD<TilePartitioner, GemmPipeline, GemmEpilogue>;
  ...
  static bool run(const int8_t *A, const int8_t *B, const float *xs,
    ...
    if (bias == nullptr) {
      ...
      broadcast_col<<<bcast_blocks, bcast_threads, 0, stream>>>(
...
bool launch_cutlass_int8_dequant(const void *A, const void *B, const void *xs,
  ...
  try {
    ...
    case 0:
      return comfy::dispatch_fused_ck<float>(a, b, x, w, bs, (float *)D, M, N,
                                             K, stream);
    case 1:
